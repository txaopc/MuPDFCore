...
namespace MuPDFCore.MuPDFRenderer
{
    ...
    public partial class PDFRenderer : Control
    {
        ...
        /// </summary>
        protected MuPDFStructuredTextPage StructuredTextPage = null;
        ...
        private void ControlPointerReleased(object sender, PointerReleasedEventArgs e)
        {
            if (SignEnabled)
            {
                ...
                //Debug.WriteLine($"Checking point [{pagePoint.X}, {PageSize.Height - pagePoint.Y}] for signature fields on page {PageNumber}");

                SignClicked?.Invoke(this, new SignClickedEventArgs(pagePoint.X, PageSize.Height - pagePoint.Y, PageNumber));
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private async void ControlPointerWheelChanged(object sender, PointerWheelEventArgs e)
        {
            if (ZoomEnabled && e.KeyModifiers.HasFlag(KeyModifiers.Control))
            {
                ZoomStep(e.Delta.Y, e.GetPosition(this));
            }
            else
            {
                if (_lastDeltaY > WHEEL_MOVEMENT_COUNT)
                {
                    _lastDeltaY = 0;
                    int page = (e.Delta.Y > 0) ? PageNumber - 1 : PageNumber + 1;
                    if (page < 0 || page >= Document.Pages.Count)
                    {
                        return;
                    }

                    if (!IsViewerInitialized || Document == null) return;

                    await InitializeAsync(Document, RenderThreadCount, page, 1.0, true, null);
                }
                else
                {
                    _lastDeltaY += 1;
